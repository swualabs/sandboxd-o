...
func (s *Service) provisionSandboxSync(ctx context.Context, sbx *model.Sandbox, req model.CreateSandboxRequest) (*model.Sandbox, error) {
	...
	netnsPath, assignedIP, err := s.setupSandboxCNI(ctx, sbx.ID)
	if err != nil {
		sbx.Error = err.Error()
		return nil, err
	}
	...
	for _, c := range req.Containers {
		...
		if sharedNetnsPath == "" && st.TaskPID > 0 {
			sharedNetnsPath = fmt.Sprintf("/proc/%d/ns/net", st.TaskPID)
		}
	}
...
func (s *Service) setupSandboxCNI(ctx context.Context, sandboxID string) (string, string, error) {
	netnsPath, err := network.EnsureSandboxNetNS(sandboxID)
	if err != nil {
		return "", "", fmt.Errorf("create sandbox netns: %w", err)
	}

	ip, err := network.CNIAdd(ctx, sandboxID, netnsPath, s.cniConfPath)
	if err != nil {
		network.DeleteSandboxNetNS(sandboxID)
...
func (s *Service) cleanupSandboxCNI(ctx context.Context, sandboxID string) {
	netnsPath := network.SandboxNetNSPath(sandboxID)
	network.CNIDel(ctx, sandboxID, netnsPath, s.cniConfPath)
	network.DeleteSandboxNetNS(sandboxID)
