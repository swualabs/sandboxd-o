...
func writeConfig(t *testing.T, raw string) string {
	t.Helper()
	path := filepath.Join(t.TempDir(), "sbxorch.json")
...
func TestLoad_DefaultAndJSON(t *testing.T) {
	cfgFile := writeConfig(t, `{"http_addr":":18082"}`)
	t.Setenv("ORCH_SQLITE_PATH", filepath.Join(t.TempDir(), "orch.db"))
