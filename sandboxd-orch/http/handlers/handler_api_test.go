...
func TestHandlers_AllEndpoints(t *testing.T) {
	...
	must(http.MethodPost, "/nodes", []byte(`{"id":"n2","metadata":{"labels":{"node-type":"example","region":"ap-northeast-2"}},"spec":{"ip":"127.0.0.1","port":18080,"unschedulable":true}}`), 200)
	must(http.MethodPost, "/nodes", []byte(`{"id":"","spec":{"ip":"127.0.0.1","port":18080}}`), 400)
	must(http.MethodPatch, "/nodes/n1", []byte(`{"spec":{"unschedulable":true}}`), 200)
	must(http.MethodPatch, "/nodes/n1", []byte(`{"metadata":{"labels":{"env":"prod","region":null}},"spec":{}}`), 200)
	must(http.MethodPatch, "/nodes/n1", []byte(`{"spec":{}}`), 400)
	must(http.MethodPatch, "/nodes/no", []byte(`{"spec":{"unschedulable":false}}`), 404)
	must(http.MethodPatch, "/nodes/n1", []byte(`{"spec":{"unschedulable":"x"}}`), 400)
	...
	must(http.MethodPost, "/sandboxes", []byte(`{"id":"obj-1","spec":{"egress":true,"node_selector":{"node-type":"example"},"containers":[{"name":"app","image":"nginx","resource":{"cpu":"100m","memory":"64Mi","ephemeral_storage":"96Mi"}}]}}`), 201)
