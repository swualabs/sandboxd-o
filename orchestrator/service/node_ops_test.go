...
func (r *fakeRepo) UpsertNode(ctx context.Context, name, ip string, port int, source string) error {
	...
	n.SandboxdBaseURL = "http://" + ip + ":" + "18080"
	r.nodes[name] = n
	return nil
}
...
func TestProbeNodeStateTransitionsAndResourceSync(t *testing.T) {
	...
	n := types.Node{Name: "n1", SandboxdBaseURL: ready.URL, State: types.NodeStateUnknown}
	r := newFakeRepo(n)
	...
	bad := types.Node{Name: "n2", SandboxdBaseURL: "http://127.0.0.1:1", State: types.NodeStateUnknown}
...
func TestLoopsAndForEachNode(t *testing.T) {
	...
	r := newFakeRepo(
		types.Node{Name: "n1", SandboxdBaseURL: server.URL},
		types.Node{Name: "n2", SandboxdBaseURL: server.URL},
	)
